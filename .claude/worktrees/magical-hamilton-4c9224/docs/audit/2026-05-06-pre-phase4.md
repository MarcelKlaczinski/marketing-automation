...
**Status**: All 9 issues resolved as of 2026-05-06. Issues #1 + #2 fixed during initial audit review (commit f576f94); Issues #3–#9 fixed in cleanup sweep e41cd70…e4141d8.
...
### Issue #1 — CRITICAL: PageSpeedValidationPipeline Not Registered in Worker ✅ Fixed
...
```
**Resolved:** commit f576f94

### Issue #2 — astro_sync_runs rows not settled on pipeline failure ✅ Fixed
...
### Issue #3 — `linking` article status is orphaned ✅ Fixed
...
**Fix:** Removed `"linking"` from the enum. Migration `0008_remove_linking_status.sql` applies the choreography-style enum rebuild.  
**Resolved:** commit 4ecc4af

### Issue #4 — `noUnusedLocals` not enabled ✅ Fixed
...
**Fix:** Added `"noUnusedLocals": true, "noUnusedParameters": true` to `tsconfig.base.json`. Fixed 5 offenders: removed unused `ReplicateGenerationError` import, `ArticleOutlineSchema` import, renamed `ctx` → `_ctx` in `OrchestrateClusterRebuildStep.execute`, removed unused `sessions` import and `userId` variable in auth test.  
**Resolved:** commit 3bee8da

### Issue #5 — ElevenLabs in costServiceEnum but no adapter ✅ Fixed
...
**Fix:** Removed `"elevenlabs"` from `costServiceEnum` (migration `0009_remove_elevenlabs_service.sql`) and removed `elevenlabsCostEur()` + `ELEVENLABS_PRICING_USD_PER_1K_CHARS` from `pricing.ts`. Updated test file accordingly.  
**Resolved:** commit 7d5a7ed

### Issue #6 — `resendCostEur` function name mismatch with `smtp` enum ✅ Fixed
...
**Fix:** Renamed `resendCostEur` → `smtpCostEur` and `RESEND_PRICING_USD_PER_EMAIL` → `SMTP_PRICING_USD_PER_EMAIL`. Updated test file.  
**Resolved:** commit aa0069b

### Issue #7 — Redis not in docker-compose.yml ✅ Fixed
...
**Fix:** Added `redis:7-alpine` service with port 6379, healthcheck, and persistent `redis_data` volume.  
**Resolved:** commit e41cd70

### Issue #8 — Article action HTTP endpoints are unauthenticated ✅ Fixed
...
**Fix:** Applied `articleRoutes.use(requireAuth)` at router level — all 4 endpoints now return 401 for unauthenticated requests. Documented in `apps/api/CLAUDE.md`.  
**Resolved:** commit e4141d8

### Issue #9 — `flux-2-pro` in pricing but not in REPLICATE_MODELS ✅ Fixed
...
**Fix:** Removed `"black-forest-labs/flux-2-pro"` from `REPLICATE_PRICING_USD_PER_IMAGE`. `flux-1.1-pro` is the production choice per Spec 12.  
**Resolved:** commit c7430e5
