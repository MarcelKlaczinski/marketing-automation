...
import { articles, clusters, contentPillars, costLogs, db, projects } from "@marketing-auto/db";
import { eq } from "drizzle-orm";
import { ArticleDraftPipeline, ArticleOutlinePipeline } from "../../src/article/pipeline.ts";
...
describe.skipIf(!LIVE)("Article Pipeline — full integration (Job 1 + Job 2)", () => {
  ...
  it(
    "Job 1: outline pipeline runs to completion and persists outline",
    async () => {
    ...
    5 * 60 * 1000
  ); // 5 min timeout — DataForSEO + Anthropic

  it(
    "Job 2: draft pipeline runs to completion and persists full article",
    async () => {
      const result = await runPipeline(
    ...
    10 * 60 * 1000
  ); // 10 min timeout — draft + image + review
