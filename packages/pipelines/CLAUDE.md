...
**`scoreBreakdown` in `trendMetadata` uses camelCase, not snake_case:** `emit-brief.ts` remaps `ScoreBreakdown` (snake_case: `community_buzz`, `serp_volatility`, etc.) to camelCase (`communityBuzz`, `serpVolatility`) when building the `trendMetadata` JSON. It also omits `source_diversity` and `total` from the stored object. Any Zod schema or TypeScript type that models `trendMetadata.scoreBreakdown` must match `emit-brief.ts` output (camelCase, 5 fields), NOT the `ScoreBreakdown` type from `types.ts`.
