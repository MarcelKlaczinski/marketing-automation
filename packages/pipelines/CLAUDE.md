...
- DO NOT set a step's `estimatedCostEur()` to a worst-case upper bound and leave it un-calibrated — `estimateWeeklyPlanCost`'s Tier-1 always wins over Tier-2 (30d historical) when ANY step returns >0, so a stale upper-bound silently dominates and triggers false-positive `BudgetExceededError` at plan-generation time. Pattern when adding a new step: ship the conservative value initially, then within 30 days of real data run `bun --env-file .env apps/api/src/scripts/discovery/check-cost-per-step.ts`, recalibrate to `real_avg × 1.4-1.7` margin, and annotate with a one-line comment `// Calibrated against <tenant> 30d data (<date>): real avg €X, ×N margin, reason`. Worst offender pre-recalibration (2026-05-27): `DraftStep` at 0.80 vs real €0.165 (~5× drift) caused €50-budget overruns for Marcel's typical 30-item plans even though actual weekly spend was ~€2.
...
1. **Widened `BaseStep.shouldRun(ctx)` → `shouldRun(ctx, input)`** in [`packages/pipelines/src/engine/step.ts`](src/engine/step.ts). Runner passes `stepInput` as the second arg in [`runner.ts:515`](src/engine/runner.ts:515). Existing `SocialGenerationStep` override (the only pre-existing shouldRun caller) ignores the second arg and remains valid under TypeScript's bivariant method rule. Forward-compatible: any future shouldRun gate can branch on step-input fields without DB queries.
2. **Added `templateKeyOverride: z.string().nullable().optional()`** to `LoadArticleOutputSchema` so `SocialImagePipeline.bridge()`'s "first-missing-field" injection (Spec 65.8 Day-5-followup) survives Zod stripping at the `load-article → extract-tools` boundary. Pre-fix, `templateKeyOverride` only reached `RenderSlides` by `.passthrough()` accident.
3. **Widened `extractedTools` from `.min(1)` → `.min(0)`** so the Family-B `skipOutput` path can produce a Zod-valid empty array. Family-A consumers never reach the skip path (their shouldRun returns true); Family-B downstream consumers (`StageFamilyBImagesStep` → Family-B `RenderSlides` branch) build their composition from `domain_extras.familyBImages` + `format_config.toolToFeature`, never from `extractedTools`.

`ExtractToolsStep` then adds `shouldRun()` returning `!isFamilyBTemplate(input.templateKeyOverride ?? null)` + `skipOutput()` returning `{...input, extractedTools: [], coverEyebrow: "", ...}` for the Family-A field stubs. Verified live: re-enqueue of Marcel's failed ChatGPT-story-arc-clickbait run completed all 7 steps (extract-tools cleanly skipped via Pattern 102), social_post rendered, article published.

**Reusable pattern — Pattern 102 with input access**: when a step that runs before a family/template branch needs to skip for a subset of cases driven by upstream `pipelineInput` fields, plumb the field through via `Pipeline.bridge()` first-missing-field injection (Spec 65.8 Day-5-followup), declare the field on the relevant intermediate schemas (`.optional().nullable()` so old runs without it parse), and use `shouldRun(ctx, input)` to branch. Avoid DB queries inside `shouldRun()` when an upstream-input check is sufficient — cheaper + side-effect-free + replay-stable. Tests at [`extract-tools-family-b-skip.test.ts`](test/article/social-image/extract-tools-family-b-skip.test.ts) cover all 6 templateKey states (3 Family-B → false, 2 Family-A → true, null/undefined → true) without DB or LLM mocks.
