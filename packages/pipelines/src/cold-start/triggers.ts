...
export async function enqueueColdStartVoiceQuestions(input: {
  projectId: string;
  ...
  const runId = await createQueuedRun(
    ...
    pipelineInput as Record<string, unknown>
  );
...
export async function enqueueColdStartCompetitorQuestions(input: {
  projectId: string;
}): Promise<TriggerResult> {
  const projectSlug = await getProjectSlug(input.projectId);
...
export async function enqueueColdStartGoLiveChecklist(input: {
  projectId: string;
}): Promise<TriggerResult> {
  const projectSlug = await getProjectSlug(input.projectId);
  const pipelineInput = { projectSlug, projectId: input.projectId };
