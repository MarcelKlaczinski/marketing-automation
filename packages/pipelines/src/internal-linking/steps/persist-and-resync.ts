...
> {
  ...
  async execute(input: z.infer<typeof InputSchema>, ctx: StepContext) {
    ...
    if (input.triggerResync) {
      try {
        const [a] = await db.select({ status: articles.status })
          .from(articles).where(eq(articles.id, input.articleId)).limit(1);
        if (a && (a.status === "ready_to_publish" || a.status === "published")) {
          await db.update(articles).set({ status: "ready_to_publish" })
            .where(eq(articles.id, input.articleId));
        ...
        ctx.log.error({ articleId: input.articleId, err: e }, "Failed to enqueue re-sync after link update");
      }
    }
