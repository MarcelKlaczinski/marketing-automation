...
- DO NOT add a query helper that drives a worker tick (or any deterministic-replay workflow) with a `.orderBy(singleSortColumn)` whose values may collide across rows — without a unique tiebreaker the rows that share the same sort value come back in arbitrary order on each tick, and any test or downstream logic that assumes "earliest-created first" or similar will flake. Same rule as cursor pagination on the API side (root CLAUDE.md "Cursor pagination needs a unique tiebreaker"). Canonical fix: `listInventoryDueForRefresh` in [helpers/content-source-inventory-read.ts](src/helpers/content-source-inventory-read.ts) orders by `(lastFetchedAt ASC NULLS FIRST, createdAt ASC, id ASC)` — `lastFetchedAt` is the semantic priority, `createdAt + id` is the stability layer. Adding the tiebreaker is strictly additive at the SQL level (PG ignores extra ORDER BY when the primary key is unique enough) but turns flaky-passing tests into deterministic-passing ones. Spec 64.20 A2 T2 worker-test fix.
- DO NOT add a tombstone-status value (`superseded`, `archived`, `deleted-soft`, etc.) without filtering it out at EVERY read site that drives incremental decisions — fixing just the write site (e.g. `INSERT ... ON CONFLICT` with `targetWhere`) leaves read-driven steps (filter-by-filepath/gitSha-equality, change-detection, gap-detection, etc.) seeing the tombstones as if they were active. Symptom: a healed-once row never re-heals because a stale tombstone's denormalised attribute (filePath / heroImageR2Key / etc.) short-circuits the read step's "this is unchanged" check. Caught in Spec 005 IR1: the partial unique index on `articles` (migration 0103) fixed the upsert path, but `FilterChangedFilesStep` still indexed superseded rows by `filePath` and skipped the incoming file via gitSha-equality. Fix: `ne(articles.status, 'superseded')` in the SELECT. Same pattern applies whenever a status enum gains a tombstone value — audit all `db.select()` sites that key on a denormalised field and add the filter.
