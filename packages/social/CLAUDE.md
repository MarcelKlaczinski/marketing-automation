...
| `tool-tier-ranking` | **1 (single still — Spec 65.17)** | 3 horizontal tier-lanes (Spitze · Stark · Solide) with 1–2 tool logos per lane, color-coded gold/silver/bronze | recurring-content articles with `domain_extras.recurring.formatConfig.tierData` (3–5 tier-mapped tools), produced by `top_n_comparison` brief-generator when `tierMode: true` |
...
**V1.6.1 splitter-terminator** (Spec V1.6.1) — `splitNarrativeByBeats` stops the per-beat capture on any line matching `/^<[A-Z][A-Z0-9_]*>/` so the trailing `<CAPTION>…</CAPTION>` + `<HASHTAGS>…</HASHTAGS>` blocks don't leak into the last beat's text. Lowercase `<em>`/`<strong>` still pass through as legitimate inline content. Defense-in-depth scrub at the next layer (`extractNarrative` in `packages/pipelines/.../family-b-render.ts`) regex-strips any tags the primary splitter missed. See root CLAUDE.md DO-NOT for the full rule.

**V1.6.1 single-tool resolver** (Spec V1.6.1) — `packages/social/src/templates/lib/load-family-b-tool.ts` is the shared template-side DB resolver consumed by both `lifestyleListicle.ts` + `opinionRecommendation.ts`. Uses `await import("@marketing-auto/db")` lazy-import (cold-path-cheap — templates must not pull pipelines + Anthropic into the renderer bundle at registry-import time). Pairs with the brief-generator-side `apps/api/.../shared/resolve-featured-tool.ts` (static import; canonical fix path). Templates fall back to `article.title ?? article.slug` when both layers miss — NEVER a literal natural-language sentinel like `"the tool"` (root CLAUDE.md DO-NOT).
