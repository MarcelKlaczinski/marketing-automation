...
- `buildInput()` reads `article.frontmatterExtras` directly — no Anthropic calls
...
**Question: do generated articles have `pros`, `cons`, `pricing`, `features`, `useCases` in `frontmatterExtras`?**
...
```
<!-- FRONTMATTER_EXTRAS: { "useCaseVerdicts": [...], "toolSlugs": [...], "winner": "...", "verdict": "...", "testMethodology": "..." } -->
```
...
**THE GAP:** A programmatically generated `tools` article (not Astro-imported) will have minimal `frontmatterExtras` and will fail the `single-tool-spotlight` eligibility check (`pros.length < 2`).
...
- `buildInput()` reads `article.frontmatterExtras` + optional DB lookup (tool icons via `buildToolLookup`)
...
| `packages/pipelines/src/article/steps/draft.ts` | Article generation — writes `frontmatterExtras` |
| `packages/adapters/astro-sync/src/import/parse-frontmatter.ts` | Astro import → `frontmatterExtras` |
