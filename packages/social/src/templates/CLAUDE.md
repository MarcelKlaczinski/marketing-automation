...
**`brandTokens` is not in scope inside `generateContent()` — only inside `render()`** (Spec 65.7 review-task). The template's `generateContent(article, input, locale, llmCaller)` signature has no brand-tokens access; the LLM-fallback `caption` it returns can't read `brandTokens.social.websiteUrl`. Two acceptable patterns: (a) emit a V1 single-tenant literal (`toolwiki.ai/${slug}`) in the generateContent-fallback path with a justification comment — render() will overwrite with the LLM caption when it succeeds; (b) at `render()`-time use the `resolveArticleUrl(brandTokens, articleSlug)` helper in `compositions/_shared/family-a/helpers.ts` for the `caption ?? fallbackCaption(...)` branch so the render-fallback path stays multi-tenant safe. Never hardcode `toolwiki.ai/...` inside `render()` — that bypasses the per-project `brandTokens.social.websiteUrl` and breaks Bellemann / Balcony-Solar tenants silently. Canonical example: all 4 Spec 65.7 Family A templates (`comparisonGrid3.ts:198` LLM-fallback uses literal, `comparisonGrid3.ts:548` render-fallback uses `resolveArticleUrl`).
