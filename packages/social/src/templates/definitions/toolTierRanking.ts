...
export const toolTierRankingTemplate: TemplateDefinition<TierRankingContext> = {
  ...
  defaultSlideCount: 1,
  // Pure data-driven — `generateContent` skips the LLM (no hook generation),
  // and `render` is just Remotion. Zero external-API cost at render time;
  // brief-generation costs are tracked under existing 65.5 cost-ops.
  estimatedCostUsd: 0,
