...
**Companion signature change (Spec 64.7):** `nanoBananaImageCostEur({model, resolution, count, mode?})` widened with an optional `mode: "sync" | "batch"` arg. Defaults to `"sync"`. `"batch"` swaps to the precomputed `NANO_BANANA_*_BATCH_PRICING_USD` maps (50% of sync per Google's documented Batch API discount). `estimateHeroImageCost(provider, resolution, mode?)` in `@marketing-auto/core/cost` mirrors the same default. Callers wanting the batch discount must pass `mode: "batch"` explicitly — no auto-detection from the snapshot here, because the same step can dispatch sync OR batch depending on per-call payload (single image vs. batch submission).
