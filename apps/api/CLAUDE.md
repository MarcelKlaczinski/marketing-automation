...
- **`shared/pick-tools.ts`** — `pickToolsForBrief()`. Manual override path (when `format_config.manualToolIds` set, verifies project + collection then truncates to topN). LLM-curated path (oversample×4 → optional `pickPersonaScoredTools` reorder when `config.persona` set → Haiku 4.5 + jsonMode picks final N → hallucination fallback drops invalid UUIDs + top-ups from pool head). Cost-tracked under `COST_OPS.RECURRING_TOOLS_CURATE` (€0.005). **Spec 65.5-followup (2026-05-27):** `loadCandidatePool` filters at SQL level via `EXISTS (SELECT 1 FROM tool_brand_assets WHERE tool_id = articles.id AND logo_url IS NOT NULL)` so the LLM only sees logo-having tools. Closes a live-launch gap where Toolwiki's 35% `deterministic-avatar` coverage caused lifestyle+top-n fires to skip 100% of the time (LLM consistently picked at least one no-logo tool, post-check rejected the brief). Manual-override path stays unfiltered (Marcel-controlled — the post-check still gates render time). Tests at [`pick-tools-brand-filter.test.ts`](../../test/lib/recurring-content/pick-tools-brand-filter.test.ts).
...
- **`shared/persist-brief.ts`** — `persistRecurringBrief()` writes `topic_briefs` with `source='recurring'`, `clusterAction='standalone'`, `approval_status='plan_pending'`, frozen `recurringMetadata` snapshot (`definitionId + runNumber + previousToolIds + formatType + formatConfig.{outputTargets, selectedTemplateKey, selectedTemplateVia, selectedEndSlide, hookData, toolIds}`). Always runs through `TopicBriefInsertSchema.parse` to enforce the `source='recurring' ⇔ recurringMetadata != null` superRefine invariant. Spec 65.9 added the optional `selectedEndSlide: {endSlideDefinitionId, type, config, name, selectedVia}` field — frozen so the 65.7 renderer reads the same shape it would have gotten from a fresh selector call (replays don't drift). **V1.6.1** added the optional `singleTool: { key: "featuredTool" | "recommendedTool"; payload: {...} }` discriminator — conditional-spreads `[input.singleTool.key]: input.singleTool.payload` into the frozen `formatConfig` so the template's `buildInput()` reads the pre-resolved object instead of doing a runtime DB lookup. Companion helper `shared/resolve-featured-tool.ts` does the DB resolve (static `import { db, articles, eq, toolBrandAssets } from "@marketing-auto/db"` — apps/api always has DB context, different from the template-side `await import` lazy pattern in `packages/social/.../templates/lib/load-family-b-tool.ts`). Both Family-B brief-generators (lifestyle-listicle + opinion-recommendation) call it at persist time. See root CLAUDE.md DO-NOT for the natural-language-sentinel anti-pattern this fix addresses.
...
**Spec 65.17 tier-mode extension** (top-n-comparison only) — when `formatConfig.tierMode === true`, the `top-n-comparison` brief-generator: (a) clamps `topN` to `3-5` (Marcel-decision Q7) with a warn-log when clamped, (b) passes `requirePersonaScores: true` through to `loadCandidatePool` so the SQL pool is pre-filtered to tools with at least one `tool_persona_scores` row (matches the brand-asset pre-filter discipline from Spec 65.5-followup — never post-check what SQL can structurally exclude), (c) computes mean-across-personas via the local `computeAggregatePersonaScores(toolIds)` helper (Marcel-decision Q8 — aggregate V1.6, per-persona deferred to V2), (d) calls `deriveTiers(scoredInput)` from [`derive-tiers.ts`](src/lib/recurring-content/derive-tiers.ts) which tercile-splits by score-rank into positive tiers `solide`/`stark`/`spitze` (Marcel-decision Q2 — never "Bad/Schlecht" because Toolwiki may have affiliate relationships), (e) forces `selectedTemplate` to `{templateKey: "tool-tier-ranking", selectedVia: "fixed"}` bypassing the 3-Layer selector entirely, (f) threads the tier-assignment through new optional `PersistRecurringBriefInput.tierData` field which spreads it into `recurringMetadata.formatConfig.tierData` for downstream replay. Tier-color tokens (gold/silver/bronze) are template-private and live alongside the composition in [`packages/social/src/compositions/tool-tier-ranking/tier-colors.ts`](../../packages/social/src/compositions/tool-tier-ranking/tier-colors.ts), NOT in global `DsTokens` — template-specific semantic overlays don't belong in the cross-template brand-token surface. Same shape reusable for any future "data-driven N-of-M tier template" — add `<format>.<flag>Mode` to that format-type's Zod config, gate the SQL pre-filter, compute the data-product, override selectedTemplate.
...
### social-render worker template dispatch (Spec 65.7-followup)

`apps/api/src/workers/social-render.worker.ts` dispatches by `templateKey` through a **set + switch** pattern. The flow:

1. **`renderSlidesViaRemotion(data)`** builds `fullInput` from the persisted `social_posts.content.renderInput` snapshot — family-specific:
   - `isFamilyB(templateKey)` → snapshot has `kind: "family-b"` + nested `compositionInput`; spread that with fresh `brandTokens` + `overrides` from job-data.
   - `isFamilyA(templateKey)` → snapshot is flat (snapshot IS the composition input); spread with fresh `brandTokens` + `overrides`.
   - Neither → `throw new Error('Unknown templateKey: ${templateKey}')`.
2. **`dispatchByTemplateKey(templateKey, fullInput, renderServer)`** (pure, exported) does the set-membership guard + per-family `switch` with an `assertNever` default. Routes to the matching `render*` function on `@marketing-auto/social/render-server`.

The two dispatch sets (Spec 65.7-followup):

| Set | Templates | Snapshot shape | Render fns |
|---|---|---|---|
| `FAMILY_A_TEMPLATE_KEYS` | `comparison-grid-3` / `comparison-grid-4` / `comparison-grid-5` / `verdict-per-use-case` / `single-tool-spotlight` / `pro-con-verdict` / `head-to-head-vs` / `head-to-head-deep-dive` | Flat — snapshot IS the composition input | `renderComparisonGrid3` … `renderHeadToHeadDeepDive` |
| `FAMILY_B_TEMPLATE_KEYS` | `story-arc-clickbait` / `lifestyle-listicle` / `opinion-recommendation` | `{ kind: "family-b", compositionInput, ... }` | `renderStoryArcClickbait` / `renderLifestyleListicle` / `renderOpinionRecommendation` |

**Compile-time exhaustivity guard:** a `type _UnhandledTemplateKey = Exclude<TemplateKey, FamilyA \| FamilyB \| Unsupported>` plus `_AssertNever<_UnhandledTemplateKey>` on a module-level const causes a TS error ("Type 'X' does not satisfy the constraint 'never'") when a new value lands in the `TemplateKey` union without being assigned to one of the three sets. `UnsupportedTemplateKey` lists keys present in the union but not registered in `bootstrap.ts` (currently `news-slide` / `concept-explainer-deck` / `price-comparison`) — they're explicit so the guard doesn't false-positive on future template-stubs that get added to the union before they ship.

**Runtime regression guard:** `apps/api/test/workers/social-render-dispatch.test.ts` walks `templateRegistry.list()` (post-`bootstrapTemplates()`) and asserts every registered key lives in `FAMILY_A_TEMPLATE_KEYS \| FAMILY_B_TEMPLATE_KEYS`. Catches the gap-class Spec 65.7 originally shipped — a `templateRegistry.register(...)` call without a matching dispatch branch.

**Adding a new template branch:**
1. Decide family — flat composition input (Family A) or `kind: "family-b"`-discriminated nested shape (Family B).
2. Add the key to `FAMILY_A_TEMPLATE_KEYS` or `FAMILY_B_TEMPLATE_KEYS` (the `as const satisfies readonly TemplateKey[]` narrows it for the switches).
3. Add the `render*` function to the `RenderServerLike` type at the top.
4. Add a `case "<new-key>": return renderServer.renderXxx(fullInput);` arm to the matching family's switch in `dispatchByTemplateKey`. The `assertNever` default will TS-error until the arm is added.
5. Register the template in `packages/social/src/templates/bootstrap.ts`.
6. The snapshot must already be persisted at INSERT time by the matching `RenderSlidesStep` branch — never reconstruct it from job-data alone (re-renders must use the persisted snapshot, NOT job-data).
...
- DO NOT design an LLM-curated tool/template/persona pool with ONLY a post-check that throws on selection violations — when the pool has a meaningful failure-rate for the post-check predicate (e.g. Toolwiki has 35% no-logo tools), the LLM consistently picks at least one violating item and the WHOLE downstream pipeline skips. The post-check is right for defense-in-depth (race against deletes), wrong as the primary gate. Pre-filter at SQL level using `EXISTS (SELECT 1 FROM gating_table WHERE ...)` so the LLM only sees acceptable candidates. Manual-override paths (`manualToolIds` etc.) stay unfiltered — the post-check is the authoritative gate for those. Pattern: `loadCandidatePool` in [`shared/pick-tools.ts`](src/lib/recurring-content/brief-generators/shared/pick-tools.ts) (Spec 65.5-followup, 2026-05-27) — verified live, restored 4/6 expected briefs after re-fire. Reusable for any future LLM-pick-from-pool surface where downstream demands a structural property: persona-score availability, region-licensing, paid-tier vs free, brand-asset coverage, etc.
