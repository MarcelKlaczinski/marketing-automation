...
if (!slug) {
  console.error("Usage: bun src/scripts/cold-start/05-go-live-checklist.ts <slug> [--force]");
  process.exit(1);
}
...
const requiredFiles = [
  {
    label: "Voice refinement (phase 1)",
    path: coldStartFile(slug, COLD_START_FILES.voiceRefinement),
  },
  {
    label: "Competitor analysis (phase 2)",
    path: coldStartFile(slug, COLD_START_FILES.competitorAnalysis),
  },
  { label: "Cluster plan (phase 3)", path: coldStartFile(slug, COLD_START_FILES.clusterPlan) },
  {
    label: "Cornerstone list (phase 4)",
    path: coldStartFile(slug, COLD_START_FILES.cornerstoneList),
  },
];
...
function renderChecklistMarkdown(projectSlug: string, output: GoLiveChecklistOutput): string {
