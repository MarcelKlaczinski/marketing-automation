...
- DO NOT bind a relative backend asset path (e.g. `"/renders/x.png"`, `"/uploads/y.svg"`) directly to `<img src>` — in dev the Vue server (port 3051) and the API server (port from `apps/web/.env` `VITE_API_BASE_URL`, e.g. 3050) live on different origins, AND the Vue server returns the SPA `index.html` for every unmatched path with `Content-Type: text/html`. So `<img src="/renders/x.png">` silently loads HTML, the browser can't decode it as an image, and the slot renders a broken-image placeholder with no console error. Always wrap backend-served paths with `assetUrl()` from `src/lib/asset-url.ts` — it reads `VITE_API_BASE_URL`, extracts the origin (`http://localhost:3050` in dev / `""` in prod when same-origin), and prefixes the path. Bonus footgun: a Vite `devServer.proxy` config in `quasar.config.ts` looks like the right fix BUT the port in stale comments may not match the actual API port — always grep `apps/web/.env` for `VITE_API_BASE_URL` to verify the API origin before configuring proxies or hardcoding ports. Applies to `/renders/*` (rendered slides) and `/uploads/*` (brand assets) — both served by the API via `serveStatic`. Caught Spec 63.X when verdict-per-use-case slides rendered as broken-image icons.
- DO NOT call `this.$q.dialog({...})` without `dark: true` in dark-only apps — the `Dark` plugin in `quasar.config.ts` controls the global `q-app` theme but does NOT auto-propagate into `$q.dialog` instances. A dialog without `dark: true` renders white-on-white in dark mode (title visible, message invisible). Same applies to `$q.notify({...})` if you want themed colours rather than the type-based ones. Pattern: `this.$q.dialog({ ..., dark: true, cancel: { flat: true, color: "white" } })`. The `color: "white"` on the flat cancel button keeps the label visible — without it the button text inherits the dialog's default text color which may not contrast against the cancel button background. Caught Spec 63.X on the re-render confirmation dialog.
- DO NOT discard the structured response from `POST /generate-templates` — the endpoint returns `{ jobs: Array<{ templateKey, status: "queued" | "skipped", reason? }> }` and returns HTTP 200 (not 202) when ALL jobs are skipped (template not registered, eligibility failed, etc.). A bare `await apiPost(...)` followed by an unconditional success notification lies to the user: every clicked Generate button shows "enqueued" while nothing was actually queued. Pattern: capture `const result = await apiPost<{ jobs: Job[] }>(...)`, branch on `result.jobs.filter(j => j.status === "skipped").length`, surface the first `reason` in a warning notification. See `generateTemplate()` in `ArticleSocialTab.vue` for the canonical implementation. Caught Spec 63.X when clicking "use-case-verdict" silently no-op'd because the LLM had cached a pre-Spec-59.3 template key.
...
- DO NOT use `list.find(isActive) ?? list[0] ?? null` when picking the "active" row from a paginated API response — when ALL rows are inactive (e.g. only `cancelled`/`superseded` plans exist for the week), the `?? list[0]` fallback surfaces an inactive row as if it were active and hides the empty-state UI (no Generate button). Return `null` when no active row matches so the `v-else-if="!plan"` branch fires. Canonical example: `useWeeklyPlan.ts` `activePlanFromList` returns `list.find((p) => !INACTIVE.includes(p.status)) ?? null` — caught when a cancelled plan permanently locked Generate-Button visibility for KW21 (2026-05-21).
- DO NOT use string-form `router.push('/path/x')` when navigating between sibling routes whose parent reads filter state from `route.query` — Vue Router 4 drops ALL query params on string-form push. Symptom: filter chips reset every time the user clicks a list item in a master-detail layout. Always use object-form `{ path: '...', query: this.$route.query }` to forward the existing filter state. Build a back-route the same way: enumerate `Object.entries(this.$route.query)` into `URLSearchParams` and append to the path. Caught in BriefsPage — `onSelectBrief` dropped `?source=…&readiness=…` on every brief-card click; same fix needed in `backRoute` computed for return navigation.
- DO NOT silently catch errors from `apiPost`/`apiPatch` with `catch { /* handled by api.ts */ }` when the endpoint is source/scope-specific — `apiPost` throws on non-2xx, but a bare catch swallows legitimate 404s that look identical to network errors. Symptom: action buttons (approve / dismiss / submit) appear to do nothing for valid-looking inputs. Always surface the error via `this.$q.notify({ type: 'negative', message: err instanceof Error ? err.message : 'action_failed' })`. Two specific cases of "scope-narrow endpoint": (a) the trends-specific `/trends/briefs/:id/{approve,dismiss}` hard-filters on `source='trend_discovery'` — for source-agnostic UI use the universal `/briefs/bulk-{approve,dismiss}` with a single-element array; (b) `/articles/:id/translate` rejects articles missing a body. Pick the source-agnostic endpoint when the calling page accepts mixed-source data. Caught when BriefDetailPage's Genehmigen-Button silently 404'd for every gap_analysis brief.
...
- DO NOT put client-side validation that produces user-facing error messages inside a `useSectionForm`'s `onSave` callback — `useSectionForm` (the canonical settings-form composable used by `useSettingsProjectPage`, brand-tokens etc.) only surfaces mutation errors when an explicit `onError` callback is configured ([useSectionForm.ts:50](src/composables/useSectionForm.ts:50)), and most call sites omit it. A bare `Promise.reject(new Error("..."))` from `onSave` is **silently swallowed** — the user sees no toast, no banner, no feedback at all. Also the composable layer has no access to `this.$t()` or `this.$q.notify`, so error messages end up hardcoded in one language. Pattern: validate in the Vue component's `@save="onMySave"` method where `$t` + `$q.notify` are available; surface errors via `this.$q.notify({ type: "negative", message: this.$t("…") as string })`; only call `myForm.save()` after validation passes. The composable's `onSave` then trusts the data and handles only the "shape correctly / handle disconnect-via-null" semantics. Canonical example: `onAstroSave()` in [SettingsProjectPage.vue](src/pages/settings/SettingsProjectPage.vue) wraps `astroForm.save()` for the GitHub-App / Astro-Repo editor (post-Spec-44 follow-up).
...
- DO NOT declare a nullable scalar prop as `{ type: Number, default: null }` (or `String` / `Boolean` etc.) and expect TypeScript to infer `number | null` — Vue's prop-type inference narrows to the constructor's primitive (`number`), so the parent's `:my-prop="someNullableNumber"` bind rejects with `Type 'number | null' is not assignable to type 'number'`. Use the explicit `PropType` two-step cast: `{ type: Number as unknown as PropType<number | null>, default: null }`. The `as unknown as` is the idiomatic resolution because Vue's prop-type internal expects the constructor reference, not a `PropType<T>`. Canonical example: `totalInFilter` in [`BulkSelectionBar.vue`](src/components/ui/BulkSelectionBar.vue) (Spec 64.17). Same pattern applies to `PropType<string | null>`, `PropType<boolean | null>`.
...
- DO NOT call `useQuasar()` from `setup()` and return `$q` — Quasar's plugin already exposes `this.$q` on every Options API component via `globalProperties`, so `this.$q.notify(...)` works without any setup wiring. Returning a property whose name starts with `$` from `setup()` triggers Vue's "reserved prefix" runtime warning (`setup() return property "$q" should not start with "$" or "_"`). Use `this.$q` directly in methods/computed; only call `useQuasar()` from inside a composable (`src/composables/useXxx.ts`) where `this` isn't available. Caught in PlannerPage.vue.
- DO NOT default to cursor mode when consuming an endpoint that supports both cursor and offset pagination (e.g. `/api/projects/:slug/articles`) — cursor mode on that endpoint uses strict `lt(updatedAt, cursor)` with no `id` tiebreaker, so any group of rows that shares `updatedAt` (bulk imports get PostgreSQL transaction-time and thus identical timestamps) is silently dropped at the page boundary. Symptom: infinite scroll stalls at exactly N×pageSize. Use offset (`?offset=N&limit=20`) and derive `hasMore` from the backend's `total` field instead. The `ArticlesListResponse` type in `src/types/ui.ts` honestly models both response shapes so callers can pick. See `useArticlesList.ts` for the canonical offset-mode consumer; see apps/api/CLAUDE.md "Cursor pagination needs a unique tiebreaker" for the backend-side fix when adding a new cursor endpoint.
- DO NOT use `this.$q.platform.is.dev` to gate dev-only console diagnostics — Quasar's `Platform` type does not have a `dev` field; TypeScript surfaces this as `TS2339: Property 'dev' does not exist on type ...`. Use Vite's `import.meta.env.DEV` (boolean) instead: `if (err instanceof Error && import.meta.env.DEV) console.warn(...)`. Same pattern when you want to skip a UI animation in dev or surface extra logging — `import.meta.env.DEV` is the canonical signal. Spec 64.20 A2 T2.
- DO NOT rely on `IntersectionObserver` firing again automatically after appending new content above the sentinel — the observer only emits on intersection **transitions**. If the sentinel was already `isIntersecting: true` before a fetch completes and stays visible afterwards (very likely with `rootMargin: "200px"`), no new event fires and scrolling stalls after the first auto-load. Watch the "page fetched" signal (e.g. TanStack's `isFetchingNextPage` flipping back to `false`) and re-attach the observer in a `$nextTick` callback; the fresh `observe()` re-checks intersection synchronously and re-fires if still on screen. Canonical example: `ArticlesListPage.vue` `watch.isFetchingMore` → `attachObserver()`.
- DO NOT name a Vue Options-API method `notify` on a component that also uses `this.$q.notify` — the method-name shadows the Quasar plugin call visually (TypeScript compiles fine but a code reader can't tell which is which on a quick scan) AND the method-name doesn't convey what it does. Rename to something explicit about the side-effect (e.g. `emitSavedAndClose`, `acknowledgeSaved`). The pattern matters because the AMBIGUOUS naming hides a separate UX bug: when a save flow only emits events without a `$q.notify({type: "positive", ...})` toast, the user sees the modal close + an empty grid refresh and assumes "passiert nichts". ALWAYS pair an emit-and-close method with an explicit success-toast that confirms the write landed — silent emits feel like no-ops. Canonical example: [ToolBrandAssetEditModal.vue](src/components/settings/ToolBrandAssetEditModal.vue) `emitSavedAndClose` + `$q.notify({type: "positive", message: $t("...saved")})` (Spec 65.2 /review-task — Marcel reported "Speichern passiert nichts" though save was firing correctly).
