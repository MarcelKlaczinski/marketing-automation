...
export const useColdStartStore = defineStore('coldStart', {
  ...
  actions: {
    ...
    async fetchCornerstones(slug: string): Promise<void> {
      const res = await api.get<{ ok: boolean; data: CornerstoneArticle[] }>(`/projects/${slug}/cold-start/cornerstones`);
    ...
    async triggerVoiceSynthesize(slug: string, answers: { questionIndex: number; answer: string }[]): Promise<{ runId: string; jobId: string }> {
      const res = await api.post<{ ok: boolean; data: { runId: string; jobId: string } }>(
    ...
    async triggerClusterPlan(slug: string, payload?: { contentGaps?: string[]; topicsToAvoid?: string[] }): Promise<{ runId: string; jobId: string }> {
      const res = await api.post<{ ok: boolean; data: { runId: string; jobId: string } }>(
    ...
    async cornerstoneEdit(slug: string, articleId: string, patch: { title?: string; cornerstoneKeyword?: string; metaDescription?: string }): Promise<void> {
