...
13. **Post-Re-Import self-healing on the `practice` / `practice-use-cases` consolidation — pillar resurrected, but for the right reason.** Marcel triggered the Toolwiki Re-Import 2026-05-24 after the Bucket-C apply + the Astro MDX clusterKey commit. Snapshot diff revealed `practice-use-cases` pillar was re-created by `SyncClustersFromFrontmatterStep`, and the `ki-business-2026` cluster was re-pointed BACK from `practice` → `practice-use-cases`. Initial alarm; investigation showed the consolidation was wrong-by-design from the start:

    - **Astro has TWO scoped category collections**, one per Astro `collection`:
      - `categories/knowledge/practice.md` (scope=knowledge, slug=`practice`, label="Praxis") — used by 4 ki-wissen articles
      - `categories/blog/practice-use-cases.md` (scope=blog, slug=`practice-use-cases`, label different) — used by 16 blog articles
    - These are NOT bilingual duplicates; they are two SEPARATE category taxonomies that happen to share the vocabulary "practice". The Bucket-C audit grouped them as drift because the DB `content_pillars.name` flattens both scopes into one namespace.
    - My consolidation (`practice` keep, `practice-use-cases` drop) collapsed them and re-pointed the blog-scope cluster `ki-business-2026` to the ki-wissen-scope `practice` pillar — semantically wrong.
    - Re-Import correctly self-healed: `SyncClustersFromFrontmatterStep` derives pillars from `articles.category` distinct values, so `practice-use-cases` (still in 16 MDX `categorySlug:` fields) re-created the pillar, and `ki-business-2026` (category=`practice-use-cases`) was correctly re-pointed.

    **Net Bucket-C-cleanup result post-Re-Import: pillars 29 → 20 (9 actual drift removed, 1 false-positive consolidation self-healed). All 7 EN/DE-pair drops are permanent (they were display-label drift like `Vergleiche` / `Grundlagen` — no MDX `categorySlug` uses German display labels, so the importer doesn't recreate them). The `Praxis` + `Praxis & Use Cases` drops (display-label form) are also permanent. The 3 ki-wissen pillars with `intentTaxonomyOverride` are preserved as planned.**

    **Lesson for future Bucket-C-style cleanups:** before consolidating any pillar whose name looks like a category slug, check the Astro-side `categories/<scope>/<name>.md` files for EACH scope (`blog`, `knowledge`, `tool`, `usecase`). If `categories/X/foo.md` AND `categories/Y/foo-bar.md` both exist as separate scope-canonical slugs, they are NOT duplicates — they are two-scope taxonomy with shared vocabulary. Consolidation will be undone by `SyncClustersFromFrontmatterStep` on the next Re-Import.

14. **The 1 stale EN article still showing `cluster_key='code-assistenten-2026'` is `blog/en/ai-code-assistants` (status=`superseded`).** It is a Spec 001 orphan blog article — the MDX file was deleted from the Astro repo before the importer's last run, and Spec 001 cleanup will mark it `status='superseded'`. The stale cluster_key on a superseded row is dormant (active-only queries filter it out) — no action needed.

15. **`SyncClustersFromFrontmatterStep` is a continuous source of pillar resurrection.** Documented in Spec 002 §9 R6 as a known risk. After this Re-Import, the lesson is concrete: any pillar deleted by Bucket-C cleanup will come back if the importer sees ≥1 article with that `category` value. The 7 EN/DE-pair drops survived because their German-display-name forms (`Vergleiche` etc.) are never used as `categorySlug` in MDX. `practice-use-cases` came back because it IS used in MDX. Future Bucket-D fix candidate: have the importer normalize / dedupe pillar names against a project-level whitelist before INSERT, or replace pillar-from-category auto-creation with explicit pillar definitions.
