...
## Section Q — Deviations from Spec (Discovered During Implementation)

### Q.1 `DsGlow` API — `corner` not `position`, integer `alpha`

The spec drafts throughout use `position="bottom-left"` and `alpha={0.25}` (float). The actual `DsGlow` component API uses:
- `corner="bottom-left"` (not `position`)
- `alpha={25}` as integer 0-100 (not 0.0-1.0)

**Fix:** Use `corner` and integer alpha. `DsGlow corner="bottom-left" alpha={25}`.

### Q.2 `DsFoot` props — `ctaBold`/`ctaLead` not `ctaLine1`/`ctaLine2`

Section E and L describe passing `ctaLine1`/`ctaLine2` directly to `DsFoot`. The actual component accepts:
- `ctaLead` — muted text (the URL, `g.ctaLine2`)
- `ctaBold` — bold text (the CTA phrase, `g.ctaLine1`)

**Fix:** `<DsFoot ctaLead={g.ctaLine2} ctaBold={g.ctaLine1} />` — note the intentional swap: ctaLine2 (URL) → ctaLead, ctaLine1 (CTA text) → ctaBold.

### Q.3 Icon rendering — `ToolIconImage` replaces `logoSlug` file-path approach

Section D.1 defines `logoSlug: string` on `ComparisonGrid3Tool` and Section E.2 shows `<img src={/assets/tools/${tool.logoSlug}.svg}>`. This is a pre-Spec-52a pattern. The codebase uses `ToolIconImage` with `iconSvg/iconInitials/iconHue` fields resolved earlier in `buildInput()`.

**Fix:** `Grid3Tool` carries `iconSvg?/iconInitials?/iconHue?` (resolved in `buildInput` via `buildToolLookup`). `ComparisonGrid3Generated["tools"][number]` stores these fields and passes them to `<ToolIconImage>`. No `logoSlug` field in the final implementation.

### Q.4 `safeZones.ts` not created

Section C.1 lists `safeZones.ts` as a file to copy from grid-4. Grid-4 does not have this file either — it was a spec error. Skipped.

### Q.5 Old `comparison-grid-3` definition was a full carousel template

The existing `comparisonGrid3.ts` definition (pre-Spec-60.3) used `ComparisonContext` (shared adapter type) and called `renderComparisonGrid()` (list-carousel). The spec implies adding a new composition, but the definition file required full replacement — not extension. The old `comparisonGrid3OverridesSchema` was carousel-specific (coverEyebrowLabel, deepDiveEyebrow, endSlide, layout fields) — incompatible with the new single-still shape. New override schema is different, but `.strip()` ensures stored legacy values are silently ignored.

### Q.6 Unused `locale` variable removed from worker

The pre-60.3 worker had `const locale = ...` only used by the old carousel branch. After replacing it with the renderInput snapshot pattern, `locale` became unused (TS6133). Removed.

---

## Section S — Commit Plan
