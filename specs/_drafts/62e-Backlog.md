...
### 62.5.1 Batch-aware Cost Estimation (deferred)

- Spec-Datei existiert: `specs/62.5.1-batch-aware-cost-estimate.md`
- Toolwiki aktuell sync mode, also nicht akut
- Reihenfolge: zwischen 62.7 und 62.8 implementieren (kurz vor Batch-Execution-Aktivierung)

### Deterministisches Cluster-Routing für ComparisonDiscovery

- **Kontext:** ComparisonDiscovery (Spec 63.3a/b) erzeugt Briefs mit `clusterId: null, clusterAction: 'comparison'`. Approve-Guard war strict, Variante-2-Fix lockert für `source=comparison_discovery` (2026-05-21).
- **Inkonsistenz:** 22 redaktionelle Bestands-Comparisons sind alle clustered nach `{topic}-comparisons-2026` Pattern. Neue Discovery-Briefs landen clusterless. Astro-Cross-Linking funktioniert via toolSlugs unabhängig von Cluster (verified: `RelatedComparisons.astro:25`).
- **Wann relevant:** Falls echte Cluster-Hub-Pages für Comparisons gebaut werden (`/de/vergleiche/chatbot-comparisons-2026/` etc.) oder Cluster-Detail-Pages mit Comparison-Listing erweitert werden.
- **Heuristik-Idee:** clusterKey deterministisch ableiten aus `comparisonMetadata.toolASlug + toolBSlug` Subcategory-Match. Same-subcategory → existing `{subcategory}-comparisons-2026` Cluster. Cross-subcategory → eigener Cluster oder Skip-Cluster.
- **Open questions:** Hub vs Spoke auto-routing? Cross-Subcategory Pairs (z.B. chatbots × translation): neuer Cluster oder mappen auf einen der beiden? Migration für existing clusterless Comparisons backfillen?
- **Dependency:** 2-3 Monate ComparisonDiscovery-Production-Daten sammeln um Heuristik zu validieren. ~1 Tag geschätzt nach Strategy-Entscheidung.
