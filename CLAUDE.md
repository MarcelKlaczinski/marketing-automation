...
- /specs/60.2-4-comparison-grid.md (comparison-grid-4 single-still composition: 4-tool fixed-height grid stack, 84px monospace score, top-right glow, right-anchored winner flag; ComparisonGrid4Slide + types + overrides + 3 fixtures; renderComparisonGrid4() in render-server.ts; worker snapshot pattern; bounds aligned to REMOTION.md field names heroTitle/heroSub/tools.verdict/footer.ctaLine)
