...
- DO NOT use `jsonMode: true` with `claude-sonnet-4-6` — this model rejects assistant message prefill with HTTP 400 `"This model does not support assistant message prefill"`. Omit `jsonMode` and extract JSON manually: `raw.indexOf("{")` / `raw.lastIndexOf("}")`. Use `systemSuffix: "Respond with only a valid JSON object. No markdown, no explanation."` and include an inline JSON skeleton in the user message. See `packages/pipelines/src/article/social-image/steps.ts` `GenerateCaptionStep` for the pattern.
